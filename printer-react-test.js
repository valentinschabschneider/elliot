...
class Printer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.overrideDefaultBackgroundColor =
      options.overrideDefaultBackgroundColor || false;
    this.timeout = options.timeout || 0;
    this.closeAfter =
      typeof options.closeAfter !== 'undefined' ? options.closeAfter : true;
  ...
  async render(input) {
    ...
    try {
      ...
      await page
        .evaluate(async () => {
          console.log('eval', window.PagedPolyfill);
          let done;
          ...
          console.log('before', window.PagedConfig);
  ...
  async pdf(input, options = {}) {
    let page = await this.render(input).catch((e) => {
      throw e;
    });
